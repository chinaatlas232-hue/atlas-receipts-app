...
with st.sidebar:
    ...
    uploaded_data_file = st.file_uploader("1. ملف بيانات الشحنات (تعبئة وصل اطلس.xlsx)", type=["xlsx"])
    if uploaded_data_file is not None:
        with open(shipment_path, "wb") as f:
            f.write(uploaded_data_file.getbuffer())
        st.success("تم حفظ ملف الشحنات بنجاح!")

    uploaded_template_file = st.file_uploader("2. قالب وصل التسليم (Atlas_Cargo_Delivery_Receipt.xlsx)", type=["xlsx"])
    if uploaded_template_file is not None:
        with open(template_path, "wb") as f:
            f.write(uploaded_template_file.getbuffer())
        st.success("تم حفظ القالب بنجاح!")

    uploaded_logo = st.file_uploader("3. شعار الشركة (Logo)", type=["png", "jpg", "jpeg"])
    if uploaded_logo is not None:
        with open(logo_path, "wb") as f:
            f.write(uploaded_logo.getbuffer())
        st.success("تم حفظ الشعار بنجاح!")

    uploaded_customer_file = st.file_uploader("4. ملف معلومات العملاء (coustmer info)", type=["xlsx", "csv"])
...
if active_data_file is not None and active_template_file is not None:
    try:
        ...
        table_pdf_html_component = f"""
            ...
            <body>
                <button style="background-color: #102a43; color: white; padding: 12px 24px; border: none; border-radius: 6px; cursor: pointer; font-weight: bold; font-size: 14px; font-family: Tahoma;" onclick="exportTablePDF()">📄 تصدير الجدول الحالي إلى PDF</button>
                <script>
